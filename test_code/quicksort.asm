...
addi $a3, $a3, -12  # Make stack room for three
...
addi $a3, $a3, 12   #Restoring the stack size
...
addi $a3, $a3, -16  #Make room for 5

sw $a0, 0($a3)      #store a0
sw $a1, 4($a3)      #store a1
sw $a2, 8($a3)      #store a2
sw $ra, 12($a3)     #store return address
...
forloop:
    ...
    lw $ra, 12($a3)         #return address
    addi $a3, $a3, 16       #restore the stack
    jr $ra              #junp back to the caller

quicksort:              #quicksort method

addi $a3, $a3, -16      # Make room for 4

sw $a0, 0($a3)          # a0
sw $a1, 4($a3)          # low
sw $a2, 8($a3)          # high
sw $ra, 12($a3)         # return address
...
lw $a0, 0($a3)          #restore a0
lw $a1, 4($a3)          #restore a1
lw $a2, 8($a3)          #restore a2
lw $ra, 12($a3)         #restore return address
addi $a3, $a3, 16       #restore the stack
